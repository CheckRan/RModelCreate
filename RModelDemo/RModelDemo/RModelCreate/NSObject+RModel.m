...
-(void)ZJModelWithDiconary:(NSDictionary *)dictionary
{
    ...
    NSArray * keyArray = [self.allKeysArray copy];
...
- (NSDictionary *)getAllPropertyDict{
    return [self r_DescriptionDictionaryWithDeep:YES];
}
...
-(NSDictionary *)r_DescriptionDictionaryWithDeep:(BOOL)deep
{
    unsigned int outCount;
    objc_property_t * propertyList = class_copyPropertyList([self class], &outCount);
    ...
    for (int i = 0 ; i < outCount; i++ ) {
        ...
        if (propeType.customType ||
            ...
            if (deep) {
                //递归调用
                [mutableDict setValue:[[self valueForKey:keyString] r_DescriptionDictionaryWithDeep:deep] forKey:keyString];
            }
        }
        else if ([propeType.foundationType isSubclassOfClass:[NSArray class]]){//Array
            NSArray * array = [self valueForKey:keyString];
            NSMutableArray * resultArray = [NSMutableArray array];
            if (array.count) {
                    for (id value in array) {
                        if (deep) {
                            [resultArray addObject:[value r_DescriptionDictionaryWithDeep:deep]];
                        }
                        else{
//                            [resultArray addObject:[NSString stringWithFormat:@"<%@:%p>",NSStringFromClass(value),value]];
                            //error  try to solution latter
                        }
                    }
                }
                [mutableDict setValue:resultArray forKey:keyString];
        }
        else //普通
        {
            if (keyString) {
                id value = [self valueForKey:keyString];
                [mutableDict setValue:value forKey:keyString];
            }
